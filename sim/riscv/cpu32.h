/* CPU family header for riscv32bf.
...
#ifndef CPU_RISCV32BF_H
#define CPU_RISCV32BF_H
...
do { \
riscv32bf_h_csr_set_handler (current_cpu, (index), (x));\
;} while (0)
...
#define GET_H_XLEN() riscv32bf_h_xlen_get_handler (current_cpu)
#define SET_H_XLEN(x) \
do { \
((void) 0); /*nop*/\
;} while (0)
...
CPU (h_fpr[(((index)) + (8))]) = (x);\
;} while (0)
#define GET_H_OPCODE7() 0
#define SET_H_OPCODE7(x) \
do { \
((void) 0); /*nop*/\
;} while (0)
#define GET_H_COPCODE2() 0
...
USI riscv32bf_h_pc_get (SIM_CPU *);
void riscv32bf_h_pc_set (SIM_CPU *, USI);
SI riscv32bf_h_xlen_get (SIM_CPU *);
void riscv32bf_h_xlen_set (SIM_CPU *, SI);
SI riscv32bf_h_gpr_get (SIM_CPU *, UINT);
void riscv32bf_h_gpr_set (SIM_CPU *, UINT, SI);
SI riscv32bf_h_gpr_not_zero_get (SIM_CPU *, UINT);
void riscv32bf_h_gpr_not_zero_set (SIM_CPU *, UINT, SI);
SI riscv32bf_h_gpr_not_sp_get (SIM_CPU *, UINT);
void riscv32bf_h_gpr_not_sp_set (SIM_CPU *, UINT, SI);
SI riscv32bf_h_gpr_not_zero_or_sp_get (SIM_CPU *, UINT);
void riscv32bf_h_gpr_not_zero_or_sp_set (SIM_CPU *, UINT, SI);
SI riscv32bf_h_zero_get (SIM_CPU *, UINT);
void riscv32bf_h_zero_set (SIM_CPU *, UINT, SI);
SI riscv32bf_h_sp_get (SIM_CPU *, UINT);
void riscv32bf_h_sp_set (SIM_CPU *, UINT, SI);
SI riscv32bf_h_c_gpr_get (SIM_CPU *, UINT);
...
void riscv32bf_h_copcode2_set (SIM_CPU *, SI);

/* These must be hand-written.  */
extern CPUREG_FETCH_FN riscv32bf_fetch_register;
extern CPUREG_STORE_FN riscv32bf_store_register;
...
#define EXTRACT_IFMT_C_FLWSP_VARS \
  UINT f_c_funct3; \
  UINT f_rd; \
  UDI f_uimm2_32; \
  UDI f_uimm1_121; \
  ...
  UINT f_c_opcode; \
  unsigned int length;
#define EXTRACT_IFMT_C_FLWSP_CODE \
  length = 2; \
  f_c_funct3 = EXTRACT_LSB0_UINT (insn, 16, 15, 3); \
...
#define EXTRACT_IFMT_C_FSWSP_VARS \
  UINT f_c_funct3; \
  UDI f_uimm2_82; \
  UDI f_uimm4_124; \
  DI f_uimm8_82_124_00; \
  ...
  UINT f_c_opcode; \
  unsigned int length;
#define EXTRACT_IFMT_C_FSWSP_CODE \
  length = 2; \
  f_c_funct3 = EXTRACT_LSB0_UINT (insn, 16, 15, 3); \
  f_uimm2_82 = EXTRACT_LSB0_UINT (insn, 16, 8, 2); \
  f_uimm4_124 = EXTRACT_LSB0_UINT (insn, 16, 12, 4); \
...
#define EXTRACT_IFMT_C_FSW_VARS \
  UINT f_c_funct3; \
  UDI f_uimm3_93; \
  UDI f_uimm1_51; \
  UDI f_uimm3_123; \
  ...
  UINT f_c_opcode; \
  unsigned int length;
#define EXTRACT_IFMT_C_FSW_CODE \
  length = 2; \
  f_c_funct3 = EXTRACT_LSB0_UINT (insn, 16, 15, 3); \
  f_uimm3_93 = EXTRACT_LSB0_UINT (insn, 16, 9, 3); \
  f_uimm1_51 = EXTRACT_LSB0_UINT (insn, 16, 5, 1); \
